...
// which are part of the Thought repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="thought-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /thought-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "thoughtnetwork-thought/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from thoughtnetwork-thought/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'thoughtnetwork-thought/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from thoughtnetwork-thought/develop"
            }
          }

          if (hasCache) {
            sh "cd /thought-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /thought-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /thought-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /thought-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /thought-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /thought-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /thought-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
