...
AC_DEFUN([THOUGHT_QT_FAIL],[
  if test "$thought_qt_want_version" = "auto" && test "$thought_qt_force" != "yes"; then
    if test "$thought_enable_qt" != "no"; then
...
AC_DEFUN([THOUGHT_QT_CHECK],[
  if test "$thought_enable_qt" != "no" && test "$thought_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([THOUGHT_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [
     ...
     if test "$thought_qt_want_version" = "yes"; then
...
AC_DEFUN([THOUGHT_QT_CONFIGURE],[
  ...
  dnl the final binary as well. _THOUGHT_QT_CHECK_STATIC_PLUGIN does
  ...
  if test "$thought_cv_static_qt" = "yes"; then
    _THOUGHT_QT_CHECK_STATIC_LIBS
    ...
    if test "$TARGET_OS" != "android"; then
      _THOUGHT_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "$TARGET_OS" = "windows"; then
      ...
      _THOUGHT_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      _THOUGHT_QT_CHECK_STATIC_PLUGIN([QWindowsVistaStylePlugin], [-lqwindowsvistastyle])
      AC_DEFINE([QT_QPA_PLATFORM_WINDOWS], [1], [Define this symbol if the qt platform is windows])
    elif test "$TARGET_OS" = "linux"; then
      _THOUGHT_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _THOUGHT_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _THOUGHT_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  THOUGHT_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  if test $thought_enable_qt = "yes"; then
...
dnl
dnl _THOUGHT_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_THOUGHT_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); THOUGHT_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _THOUGHT_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_THOUGHT_QT_CHECK_STATIC_LIBS], [
