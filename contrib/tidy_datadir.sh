#!/bin/bash
# Copyright (c) 2013 The Thought Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Thought database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Thought datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Thought datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Thought 0.7 datadir."
    ;;
  3)
    echo "Detected Thought pre-0.8 datadir."
    ;;
  4)
    echo "Detected Thought 0.8 datadir."
    ;;
esac
