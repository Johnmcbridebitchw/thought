...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total thought balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized thought balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "debug ( 0|1|addrman|alert|bench|chainout|coindb|db|lock|rand|rpc|selectcoins|mempool"
            "|mempoolrej|midas|net|proxy|prune|http|libevent|tor|zmq|"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CThoughtAddress(addr).ToString());
...
#ifdef ENABLE_WALLET
        // Case 1: Thought address and we have full public key:
        CThoughtAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are thought addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) thought address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CThoughtAddress address(innerID);
...
{
    if (type == 2) {
        address = CThoughtAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CThoughtAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CThoughtAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CThoughtAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"balance\"  (string) The current balance in notions\n"
            "  \"received\"  (string) The total number of notions received (including change)\n"
            "}\n"
            "\nExamples:\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Thought features */
    { "thought",               "mnsync",                 &mnsync,                 true,  {} },
    { "thought",               "spork",                  &spork,                  true,  {"value"} },
