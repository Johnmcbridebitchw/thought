...
std::string CDeterministicMNState::ToString() const
{
    ...
    if (ExtractDestination(scriptPayout, dest)) {
        payoutAddress = CThoughtAddress(dest).ToString();
    }
    if (ExtractDestination(scriptOperatorPayout, dest)) {
        operatorPayoutAddress = CThoughtAddress(dest).ToString();
    }

    return strprintf("CDeterministicMNState(nRegisteredHeight=%d, nLastPaidHeight=%d, nPoSePenalty=%d, nPoSeRevivedHeight=%d, nPoSeBanHeight=%d, nRevocationReason=%d, "
        "ownerAddress=%s, pubKeyOperator=%s, votingAddress=%s, addr=%s, payoutAddress=%s, operatorPayoutAddress=%s)",
        nRegisteredHeight, nLastPaidHeight, nPoSePenalty, nPoSeRevivedHeight, nPoSeBanHeight, nRevocationReason,
        CThoughtAddress(keyIDOwner).ToString(), pubKeyOperator.ToString(), CThoughtAddress(keyIDVoting).ToString(), addr.ToStringIPPort(false), payoutAddress, operatorPayoutAddress);
}
...
void CDeterministicMNState::ToJson(UniValue& obj) const
{
    ...
    CTxDestination dest;
    if (ExtractDestination(scriptPayout, dest)) {
        CThoughtAddress payoutAddress(dest);
        obj.push_back(Pair("payoutAddress", payoutAddress.ToString()));
    }
    obj.push_back(Pair("pubKeyOperator", pubKeyOperator.ToString()));
    if (ExtractDestination(scriptOperatorPayout, dest)) {
        CThoughtAddress operatorPayoutAddress(dest);
...
std::vector<std::pair<arith_uint256, CDeterministicMNCPtr>> CDeterministicMNList::CalculateScores(const uint256& modifier) const
{
    ...
    ForEachMN(true, [&](const CDeterministicMNCPtr& dmn) {
        ...
        // TODO When https://github.com/thought/thought/pull/13191 gets backported, implement something that is similar but for single-sha256
        uint256 h;
        CSHA256 sha256;
...
bool CDeterministicMNManager::BuildNewListFromBlock(const CBlock& block, const CBlockIndex* pindexPrev, CValidationState& _state, CDeterministicMNList& mnListRet, bool debugLogs)
{
    ...
    for (int i = 1; i < (int)block.vtx.size(); i++) {
        ...
        if (tx.nType == TRANSACTION_PROVIDER_REGISTER) {
            ...
            Coin coin;
            if (!proTx.collateralOutpoint.hash.IsNull() && (!GetUTXOCoin(dmn->collateralOutpoint, coin) || coin.out.nValue != 314000 * COIN)) {
