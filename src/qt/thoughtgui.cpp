...
ThoughtGUI::ThoughtGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ThoughtGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Thought address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and thought: URIs)"));
...
#ifdef ENABLE_WALLET
bool ThoughtGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void ThoughtGUI::showMNConfEditor()
{
...
void ThoughtGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void ThoughtGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in thought
        {
