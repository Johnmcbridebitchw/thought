...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(THOUGHT_IPC_PREFIX, Qt::CaseInsensitive)) // thought: URI
        {
            ...
            {
                CThoughtAddress address(r.address.toStdString());
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(THOUGHT_IPC_PREFIX, Qt::CaseInsensitive)) // thought: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            {
                CThoughtAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const PAIRTYPE(CScript, CAmount)& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CThoughtAddress(dest).ToString()));
        }
