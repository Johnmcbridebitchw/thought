...
public:
    ...
    void fixup(QString &input) const
    {
        ...
        if(valid)
        {
            input = ThoughtUnits::format(currentUnit, val, false, ThoughtUnits::separatorAlways);
    ...
    {
        lineEdit()->setText(ThoughtUnits::format(currentUnit, value, false, ThoughtUnits::separatorAlways));
        Q_EMIT valueChanged();
    }
    ...
    void stepBy(int steps)
    {
        ...
        val = qMin(qMax(val, CAmount(0)), ThoughtUnits::maxMoney());
        setValue(val);
    }
...
private:
    ...
    {
        CAmount val = 0;
        bool valid = ThoughtUnits::parse(currentUnit, text, &val);
        if(valid)
        {
...
#include "thoughtamountfield.moc"
    ...
    amount(0)
{
    ...
    unit->setModel(new ThoughtUnits(this));
...
void ThoughtAmountField::unitChanged(int idx)
{
    ...
    int newUnit = unit->itemData(idx, ThoughtUnits::UnitRole).toInt();
...
void ThoughtAmountField::setSingleStep(const CAmount& step)
{
