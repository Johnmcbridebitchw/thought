    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "thought" is a composite category enabling all Thought-related debug output
                if(ptrCategory->count(std::string("thought"))) {
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty thought.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override thought.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", THOUGHT_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Thought Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + " 2014-2017 The Dash Core developers"
		            + "\n" + strPrefix + " 2009-2015 The Bitcoin Core developers";
    }
