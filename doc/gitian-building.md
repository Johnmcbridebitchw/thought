...
*Setup instructions for a Gitian build of Thought Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Thought
...
- [Building Thought Core](#building-thought-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-thought-host/README.md).
...
Clone the git repositories for Thought Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/thoughtnetwork/thought
...
in the Thought Core repository under 'Fetch and create inputs' to install sources which require
...
To build Thought Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Thought Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/thought/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/thoughtnetwork/thought
...
For example:
```bash
URL=https://github.com/crowning-/thought.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit thought=${COMMIT} --url thought=${URL} ../thought/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit thought=${COMMIT} --url thought=${URL} ../thought/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit thought=${COMMIT} --url thought=${URL} ../thought/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the thought git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../thought/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/thought
SIGPATH=/some/root/path/thought-detached-sigs

./bin/gbuild --url thought=${BTCPATH},signature=${SIGPATH} ../thought/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/thought-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/thought-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/thought-osx-build.assert
```
...
[thought/gitian.sigs](https://github.com/thoughtnetwork/gitian.sigs/) repository, or if that's not possible to create a pull
request.
