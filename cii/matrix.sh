...
export GOAL="install"
export SDK_URL=${SDK_URL:-https://thoughtcore.org/depends-sources/sdks}
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export THOUGHT_CONFIG="--enable-gui --enable-reduce-exports"
  ...
  export DEP_OPTS="NO_QT=1"
  export THOUGHT_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports LDFLAGS=-static-libstdc++"
  export USE_SHELL="/bin/thought"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_THOUGHT_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
